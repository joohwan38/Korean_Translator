...
class TranslationApp:
    ...
    def install_ollama(self):
        ...
        try:
            ...
            instruction_text = tk.Text(install_window, wrap=tk.WORD, width=70, height=15, padx=15, pady=15)
            ...
            if platform == "darwin":  # macOS
                ...
                4. 설치가 완료되면 이 창을 닫고 '확인' 버튼을 클릭하세요.
                """)
                ...
                instruction_text.insert(tk.END, "https://ollama.com/download", "hyperlink")
                instruction_text.insert(tk.END, """ 페이지를 방문하세요.
                2. Windows용 Ollama를 다운로드하고 설치하세요.
                3. 설치 후 Ollama 앱을 실행하세요.
                4. 설치가 완료되면 이 창을 닫고 '확인' 버튼을 클릭하세요.
                """)
                ...
                2. 설치 후 터미널에서 'ollama serve' 명령으로 Ollama 서버를 시작하세요.
                3. 또는 """
    ...
    def start_ollama(self):
        ...
        try:
            ...
            if result.returncode != 0:
                # Ollama가 설치되어 있지 않으면 설치
                result = messagebox.askyesno("Ollama 설치", "Ollama가 설치되어 있지 않습니다. 설치하시겠습니까?")
                if result:
                    if self.install_ollama():
                        self.status_label.config(text="Ollama 설치 완료, 실행 중...")
                    else:
                        return
                else:
                    self.status_label.config(text="Ollama 설치 필요")
                    return
            ...
            messagebox.showerror("오류", "Ollama 서버를 시작하지 못했습니다.")
        except Exception as e:
            self.status_label.config(text=f"Ollama 시작 오류: {str(e)}")
            messagebox.showerror("오류", f"Ollama 시작 오류: {str(e)}")
